export const mediumQs = [
...

    {
      ...
      answer: {
        ...
        vi: `## Constructor thông thường\nMỗi lần gọi Constructor, bộ vi xử lý Dart auto giật cấp phát và chẻ ra ngay lập tức khởi tạo thành 1 đối tượng mới mẻ nhồi vào Heap thay vì chờ gì.\n\n## Quyền lực Factory\nHàm hàm súc mang từ khóa \`factory\` nắm trùm khả năng là "Không rạch ròi cứ phải đẻ mới Object". Nó bọc logic phán quy:\n1. Trả lại instance nằm rêu mốc đã lưu cache bấy lâu (Lõi tối quan trọng cài chuẩn Singleton pattern).\n2. Đánh giá condition trả về cha class hay trả ngược vè đồ nhái ở Subclass (kiểu Pattern Factory xưởng chẻ nhánh con).\n3. Tối thượng là check tham số đầu vào bị lệch mã lập tức chọi Error bay thẳng đi ngay vỏ bọc khỏi tốn công đẻ thai sinh quái thai.`
      },
      level: "intermediate",
      tags: ["Dart", "Architecture"]
    },
...

    {
      ...
      answer: {
        ...
        vi: `## Định nghĩa CustomPainter\nCông cụ cắm tận rễ bộ cước vẽ (Low-level rendering). Vứt lớp hộp Widget ra, bạn cầm chổi tô màu trực diện vào bức \`Canvas\` trần trụi dưới lõi nhân engine. Tha hồ bóp nặn tạo dải tròn, uốn nét Vector hay phối lớp màu quang phổ tuỳ hứng.\n\n## Triển khai thực chiến\nTạo một file Class bôn ba Kế thừa \`CustomPainter\`. Nắm đứt cái override hàm \`paint(Canvas canvas, Size)\` để chỉ định bút tích vẽ nét xăm lên. Khoác hờ class này lồng dưới ô Widget \`CustomPaint\`.\n*Đất dụng võ hoàng kim:* Màn đồ thị phân tâm học vặn xoắn chứng khoán, khu vực màn hình kí chữ điện tử chạm vuốt cực nhạy hay render biểu đồ hạt vật lí mà nếu cẩu huyết chắp ráp từ 10.000 hộp Widget Container thì máy nổ luôn FPS sập hầm tức khắc.`
      },
      level: "intermediate",
